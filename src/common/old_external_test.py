...
class old_external_test():
        ...
        def external_testset(self,external_data):
                ...
                try:
                        ...
                        df = self.assign_clusters_to_testdata(df,num_of_clusters,cluster_centroid_file_names,cluster_label)
                except (KeyError, subprocess.CalledProcessError): #single cluster-- signle model--multiple regression
                        cluster_label = ['0']
                        num_of_clusters = 1
                        df['Class']  = cluster_label[0]
        ...
        def assign_clusters_to_testdata(self,data,num_of_centroids,cluster_centroid_file_names,cluster_label):
                ...
                for i in range(num_of_centroids): #for all clusters
                        ...
                        while(1):
                                try:
                                        dist_from_ref = joblib.load(str(self.curr_directory)+'/object_file/cluster_ref/other_refPoi_'+str(m)+'_'+cluster_label[i]+'.sav' )  #cluster  label from i and  file sm 
